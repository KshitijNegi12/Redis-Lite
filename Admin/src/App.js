...
const App = () => {
      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          if (data.type === "error") {
            ...
            setOutput((prev) => [
              ...prev,
              { timestamp, command: null, response: JSON.stringify(data) },
            ]);
          }
        } catch {
          // Handle raw message
    ...
    if (socketRef.current) {
      if (!hasSentInitial) {
        socketRef.current.send(
          JSON.stringify({ type: "init", port: redisPort }),
        );
      ...
      setOutput((prev) => [
        ...prev,
        { timestamp, command: input, response: null },
      ]);
      setInput("");
      setError(null);
    }
  ...
  const handleCopyOutput = () => {
    const textToCopy = output
      .map(
        (item) =>
          `${item.timestamp}${item.command ? ` > ${item.command}` : ""}\n${item.response ?? ""}`,
      )
      .join("\n\n");
